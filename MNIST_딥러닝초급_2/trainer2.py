...
class Trainer():
    ...
    def __init__(self, model, optimizer, crit, config): # config 확인할것
        self.model = model
        self.crit = crit
    ...
    def _train(self, train_loader, config):
        ...
        x_list = [x for x, _ in train_loader]
        y_list = [y for _, y in train_loader]
        print(len(x_list))
        print(len(y_list))
        x = torch.stack([x.clone().detach() for x in x_list])
        y = torch.stack([y.clone().detach() for y in y_list])
    ...
    def _validate(self, valid_loader, config):
        ...
        with torch.no_grad():
            x_list = [x for x, _ in valid_loader]
            y_list = [y for _, y in valid_loader]
